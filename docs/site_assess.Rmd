...
```{r, fig.height = 4, fig.width = 12}
sppsum_plo(vegdat, sp, typ = 'fo', thm)
```

And now average percent basal cover. 

```{r, fig.height = 4, fig.width = 12}
sppsum_plo(vegdat, sp, typ = 'cover', thm)
```
...
```{r, fig.height = 6, fig.width = 10}
treesum_plo(treedat, site = site, byspecies = T, var = 'trees_ha')
```

Tree summary plot, but aggregated across species.

```{r, fig.height = 6, fig.width = 10}
treesum_plo(treedat, site = site, byspecies = F, var = 'trees_ha')
```
